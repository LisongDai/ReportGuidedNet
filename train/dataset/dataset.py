...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if sample['mask_modal_idx'] != "":
            idx = sample['mask_modal_idx']
            # print(idx)
            image[idx]=np.zeros(image.shape[1:])
        # label = sample['label']
...
class BrainData(Dataset):
    def __init__(self, csv_path, np_path,report_observe,mode = 'train', augmentation = False, mask_modal=""):
        ...
        self.modal_dic = ["DWI","T1WI","T2WI","T2FLAIR","ADC"]
        self.idx = mask_modal if mask_modal == "" else self.modal_dic.index(mask_modal)
    ...
    def __getitem__(self, index):
        ...
        class_label = self.rad_graph_results[self.ann[fid]["labels_id"],:,:] 
        labels = self.triplet_extraction(class_label)

        sample = {'image': images, 'label': labels, "fid":fid, 'entity':entity,'mask_modal_idx':self.idx}
